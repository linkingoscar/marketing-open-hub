...
export function generateAnnotation(
  ...
  const isSignificant =
    apa.p.includes("<") || parseFloat(String(apa.p).replace(/[<>]/g, "")) < 0.05;
  ...
  switch (testType) {
    ...
    case "normality":
      return isSignificant
        ? {
            ...
            plainExplanation: `正态性检验表明数据分布形态显著偏离标准正态分布，后续分析需谨慎对待参数检验前提。`,
            practicalImplication: `如果样本量较大（>50）且无极端偏态，可依赖大样本性质；若偏态严重或样本量较小，建议采用非参数检验。`,
            actionItems: [
              "考虑使用 Mann-Whitney U 代替 t 检验",
              "考虑使用 Kruskal-Wallis 代替 ANOVA",
              "检查是否存在极端异常值",
            ],
            caveats: [
              "当前采用偏度-峰度探索性推断，非严格 Shapiro-Wilk",
              "大样本下对微弱偏离较敏感，需结合直方图判断",
            ],
          }
        : {
            oneLiner: `数据符合正态分布假设（p ${apa.p}）。`,
            plainExplanation: `正态性检验表明数据偏度与峰度未显著偏离正态基准，可作为参数检验前提的参考依据。`,
            practicalImplication: `满足了使用 t 检验、ANOVA 等参数检验的探索性前提。`,
            actionItems: ["可以继续使用参数检验方法", "仍需检查方差齐性等其余前提假设"],
            caveats: [
              "当前为探索性偏度-峰度检验，非严格 Shapiro-Wilk",
              "检验未达显著不代表绝对正态，仅代表未发现显著偏离证据",
            ],
          };
    ...
    case "homogeneity":
      return isSignificant
        ? {
            ...
            actionItems: [
              "使用 Welch t 检验代替标准 t 检验",
              "使用 Welch ANOVA 代替标准 ANOVA",
              "报告方差不齐的情况",
            ],
    ...
    case "mediation": {
      ...
      const mediated =
        typeof ciLower === "number" && typeof ciUpper === "number" && (ciLower > 0 || ciUpper < 0);
