...
describe("generateAnnotation", () => {
  ...
  describe("descriptive", () => {
    it("returns a generic annotation for descriptive stats", () => {
      const result = generateAnnotation(
        "descriptive",
        { mean: 3.5, sd: 1.2 },
        { p: "N/A", conclusion: "描述性统计完成" }
      );
  ...
  describe("normality", () => {
    it("detects non-normal distribution (significant)", () => {
      ...
      expect(
        result.actionItems.some((a) => a.includes("Mann-Whitney") || a.includes("非参数"))
      ).toBe(true);
    });
    ...
    for (const testType of ["ttest", "paired-ttest"]) {
      it(`${testType}: significant result`, () => {
        const result = generateAnnotation(
          testType,
          { t: 3.45, df: 98 },
          { p: "0.001", effect: "d = 0.72", conclusion: "显著" }
        );
  ...
  describe("anova", () => {
    it("significant result with post-hoc suggestion", () => {
      const result = generateAnnotation(
        "anova",
        { f: 5.67, dfBetween: 2, dfWithin: 97 },
        { p: "0.005", effect: "η² = 0.10", conclusion: "显著" }
      );
      expect(result.oneLiner).toContain("显著差异");
  ...
  describe("cfa", () => {
    it("good model fit", () => {
      const result = generateAnnotation(
        "cfa",
        { rmsea: 0.04, cfi: 0.97, srmr: 0.03 },
        { p: "N/A", conclusion: "拟合良好" }
      );
      expect(result.oneLiner).toContain("良好");
    });
    ...
    it("poor model fit", () => {
      const result = generateAnnotation(
        "cfa",
        { rmsea: 0.12, cfi: 0.82, srmr: 0.1 },
        { p: "N/A", conclusion: "拟合不佳" }
      );
  ...
  describe("mediation", () => {
    it("significant mediation (CI excludes zero)", () => {
      const result = generateAnnotation(
        "mediation",
        { indirectEffect: 0.15, ciLower: 0.05, ciUpper: 0.28 },
        { p: "N/A", conclusion: "中介显著" }
      );
      expect(result.oneLiner).toContain("显著");
    ...
    it("non-significant mediation (CI includes zero)", () => {
      const result = generateAnnotation(
        "mediation",
        { indirectEffect: 0.03, ciLower: -0.08, ciUpper: 0.14 },
        { p: "N/A", conclusion: "中介不显著" }
      );
      expect(result.oneLiner).toContain("不显著");
    });
  });
  ...
  describe("edge cases", () => {
    it("handles p-value with < prefix", () => {
      const result = generateAnnotation(
        "ttest",
        { t: 5.0 },
        { p: "<.001", effect: "d = 1.2", conclusion: "显著" }
      );
      expect(result.oneLiner).toContain("显著差异");
    });
