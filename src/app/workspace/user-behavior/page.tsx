...
export default function UserBehaviorPage() {
  ...
  const handleFileUpload = (data: ParsedData) => {
    ...
    const evt =
      headers.find((h, i) => /event|事件|action|行为|type/i.test(lc[i])) ??
      headers[1] ??
      headers[0];
    const ts =
      headers.find((h, i) => /time|date|日期|时间|timestamp|created/i.test(lc[i])) ??
      headers[2] ??
      headers[0];
    ...
    try {
      if (mode === "funnel") {
        const steps = funnelSteps
          ...
          .filter(Boolean);
        if (steps.length < 2) {
          setResult({ error: "请输入至少 2 个漏斗步骤（逗号分隔）" });
          setLoading(false);
          return;
        }
        ...
        const totalConversion = prevUsers
          ? ((prevUsers.size / (funnel[0]?.users ?? 1)) * 100).toFixed(1)
          : "0";
        const output = {
          funnel,
        ...
        const cohorts = new Map<
          string,
          { users: Set<string>; activeByWeek: Map<number, Set<string>> }
        >();
        for (const [uid, dates] of userDates) {
          ...
          if (!cohorts.has(cohort))
            cohorts.set(cohort, { users: new Set(), activeByWeek: new Map() });
        ...
        const cohortTable: CohortTableRow[] = [...cohorts.entries()]
          ...
          .map(([cohort, aggregate]) => {
            ...
            cohortPeriods.forEach((period, week) => {
              const count =
                week === 0 ? aggregate.users.size : (aggregate.activeByWeek.get(week)?.size ?? 0);
              counts[period] = count;
              periods[period] =
                week === 0 ? `${count} (100%)` : `${((count / size) * 100).toFixed(1)}%`;
            });
        ...
        const bestCohort = [...cohortTable].sort(
          (a, b) =>
            (b.counts["Week 4"] ?? 0) / Math.max(b.size, 1) -
            (a.counts["Week 4"] ?? 0) / Math.max(a.size, 1)
        )[0];
        const output = {
        ...
        for (const row of fileData.rows) {
          ...
          const value =
            typeof row[valueCol] === "number"
              ...
              : Number(row[valueCol]) || 1;
          if (!userData.has(uid))
            userData.set(uid, { lastPurchase: date, frequency: 0, monetary: 0 });
        ...
        const output = {
          ...
          avg_frequency: +(rfmData.reduce((s, r) => s + r.frequency, 0) / rfmData.length).toFixed(
            1
          ),
        ...
        if (!hasAnyKey()) {
          setResult({ error: "请先配置 API Key" });
          setLoading(false);
          return;
        }
        const sample = fileData.rows
          .slice(0, 50)
          ...
          messages: [
            {
              role: "system",
              content:
                "你是一个用户行为分析专家。根据以下用户行为数据，输出：1) 关键发现 3-5 条 2) 用户分群建议 3) 转化优化建议 4) 流失预警。用中文输出，结构化为 JSON。",
            },
            {
              role: "user",
              content: `列名: ${fileData.headers.join(", ")}\n数据样本（前50行）:\n${sample}`,
            },
          ],
          temperature: 0.3,
          maxTokens: 3000,
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-lg bg-[#3B82F6]/10 flex items-center justify-center text-xl">
            📊
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">用户行为分析</h1>
            <p className="text-sm text-[var(--text-muted)]">
              基于 ZenGrowth · Istari · E-commerce Consumer Analysis
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-6">
          上传用户行为数据（事件日志），自动完成漏斗/留存/RFM/同期群分析
        </p>
          ...
          <div className="space-y-4">
              ...
              {result && !result.error && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    a.href = url;
                    a.download = `behavior-${mode}-${Date.now()}.json`;
                    a.click();
                  }}
                >
            ...
            <div className="glass-card p-6 min-h-[400px] max-h-[700px] overflow-y-auto">
              ...
              {mode === "funnel" && result && !result.error && Array.isArray(result.funnel) && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      总转化 {String(result.total_conversion)}
                    </Badge>
                  </div>
                  {(result.funnel as { step: string; users: number; rate: string }[]).map(
                    (s, i) => {
                      const pct =
                        (s.users /
                          ((result.funnel as { step: string; users: number }[])[0]?.users ?? 1)) *
                        100;
                      return (
                  ...
                  <p className="text-xs text-[var(--text-muted)] mt-2">
                    共 {String(result.total_users)} 个独立用户
                  </p>
                </div>
              )}
              ...
              {mode === "retention" && result && !result.error && result.retention && (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-[var(--border)]">
                        ...
                        {[
                          ...
                          <th key={d} className="text-center py-2 px-2 text-[var(--text-muted)]">
                            {d}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {Object.entries(
                        result.retention as Record<string, Record<string, string>>
                      ).map(([cohort, days]) => (
                        <tr key={cohort} className="border-b border-[var(--border)]">
                          <td className="py-2 px-2 text-[var(--text-primary)] font-mono">
                            {cohort}
                          </td>
                          {[
                            "Day 0",
                            ...
                            return (
                              <td
                                key={d}
                                className={cn(
                                  "text-center py-2 px-2 text-[var(--text-secondary)]",
                                  bg
                                )}
                              >
                                {val}
              ...
              {mode === "cohort" &&
                result &&
                !result.error &&
                Array.isArray(result.cohort_table) && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-3">
                      <div className="text-center">
                        <div className="text-lg font-bold text-[var(--text-primary)]">
                          {String(result.cohorts_analyzed)}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-lg font-bold text-[var(--text-primary)]">
                          {String(
                            (result.summary as Record<string, string>)?.week_1_retention ?? "—"
                          )}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-lg font-bold text-[var(--text-primary)]">
                          {String(
                            (result.summary as Record<string, string>)?.best_retention_cohort ?? "—"
                          )}
                        </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-[var(--border)]">
                            <th className="text-left py-2 px-2 text-[var(--text-muted)]">
                              首次活跃月份
                            </th>
                            ...
                            {cohortPeriods.map((period) => (
                              <th
                                key={period}
                                className="text-center py-2 px-2 text-[var(--text-muted)]"
                              >
                                {period}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {(result.cohort_table as CohortTableRow[]).map((row) => (
                            <tr key={row.cohort} className="border-b border-[var(--border)]">
                              ...
                              <td className="py-2 px-2 text-[var(--text-secondary)] text-right">
                                {row.size}
                              </td>
                              {cohortPeriods.map((period) => {
              ...
              {mode === "rfm" && result && !result.error && result.segments && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {Object.entries(result.segments as Record<string, number>).map(
                      ([seg, count]) => {
                        ...
                        <table className="w-full text-xs">
                          <thead>
                            <tr className="border-b border-[var(--border)]">
                              ...
                              <th className="text-right py-1 px-2 text-[var(--text-muted)]">
                                消费
                              </th>
                              <th className="text-right py-1 px-2 text-[var(--text-muted)]">
                                频次
                          ...
                          <tbody>
                            {(
                              ...
                              <tr key={i} className="border-b border-[var(--border)]">
                                <td className="py-1 px-2 text-[var(--text-primary)] font-mono">
                                  {u.uid}
                                </td>
                                <td className="py-1 px-2 text-[var(--text-secondary)]">
                                  {u.segment}
                                </td>
                                <td className="py-1 px-2 text-[var(--text-primary)] text-right">
                                  {u.monetary}
                                </td>
                                <td className="py-1 px-2 text-[var(--text-secondary)] text-right">
                                  {u.frequency}
                                </td>
                                <td className="py-1 px-2 text-[var(--text-secondary)] text-right">
                                  {u.recency_days}
              ...
              {/* Error */}
              {result?.error && (
                <div className="text-[var(--error)] text-sm">{result.error as string}</div>
              )}
