...
function decompose(
  ...
  const trendDir =
    trendEnd > trendStart * 1.05 ? "上升" : trendEnd < trendStart * 0.95 ? "下降" : "平稳";
  ...
  const residualStd =
    Math.sqrt(residual.reduce((s, v) => s + (v - residualMean) ** 2, 0) / Math.max(1, n - 1)) ||
    sd ||
    1;
  ...
  const anomalies = residual
    .map((r, i) => ({
      ...
      label: labels[i],
      value: data[i],
      residual: +r.toFixed(2),
      z: +((r - residualMean) / residualStd).toFixed(2),
    }))
...
export default function TimeSeriesPage() {
  ...
  const handleRun = () => {
    ...
    const labels = validRows.map((r, i) =>
      dateCol ? String(r[dateCol]).slice(0, 10) : `T${i + 1}`
    );
    ...
    addRecord({
      tool: "timeseries",
      type: "时间序列分解",
      input: `${fileData.fileName} (${valueCol})`,
      result: JSON.stringify(res.stats),
    });
  };
        ...
        messages: [
          {
            role: "system",
            content:
              "你是一个时间序列分析专家。根据分解结果，输出：1) 趋势解读 2) 季节性模式分析 3) 异常点识别 4) 预测建议 5) 业务行动建议。用中文输出。",
          },
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-lg bg-[#F97316]/10 flex items-center justify-center text-xl">
            📈
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">时间序列分析</h1>
            <p className="text-sm text-[var(--text-muted)]">
              趋势分解 · 季节性检测 · 因果影响评估 · 预测
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-4">
          上传时间序列数据 → 自动分解为趋势+季节+残差 → AI 解读 → 生成预测建议
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            {fileData && (
              <div className="glass-card p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={period}
                      onChange={(e) => setPeriod(Number(e.target.value) || 7)}
                ...
                <p className="text-[10px] text-[var(--text-muted)]">
                  周期长度：日数据=7(周)，月数据=12(年)，按实际业务周期调整
                </p>
              </div>
            )}
            ...
            <Button
              onClick={handleAI}
              disabled={!result || loading || !hasAnyKey()}
              variant="outline"
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-[var(--border)]">
                            ...
                            <th className="text-right py-2 px-2 text-[var(--text-muted)]">
                              预测值
                            </th>
                        ...
                        <tbody>
                          {result.forecast.map((row) => (
                            <tr key={row.label} className="border-b border-[var(--border)]">
                              <td className="py-1.5 px-2 text-[var(--text-primary)] font-mono">
                                {row.label}
                              </td>
