    ...
    try {
      ...
      for (const edge of edges) {
        ...
        const xCol = numCols.find((c) =>
          c.toLowerCase().includes(fromNode.label.toLowerCase().slice(0, 4))
        );
        const yCol = numCols.find((c) =>
          c.toLowerCase().includes(toNode.label.toLowerCase().slice(0, 4))
        );
        ...
        if (!rSquared[toNode.label] || r2 > rSquared[toNode.label])
          rSquared[toNode.label] = +r2.toFixed(4);
      }
      ...
      addRecord({
        tool: "sem-editor",
        type: "SEM 路径分析",
        input: `${nodes.length} nodes, ${edges.length} edges`,
        result: JSON.stringify({
          paths: paths.length,
          sig: paths.filter((p) => p.significant).length,
        }),
      });
    } catch (e: unknown) {
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card p-4 space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <select
                  value={newType}
                  onChange={(e) => setNewType(e.target.value as Node["type"])}
                ...
                <Button size="sm" onClick={addNode} className="h-8 bg-[var(--primary)] text-white">
              ...
              <div className="glass-card p-4">
                ...
                <div className="space-y-1">
                  {edges.map((e, i) => {
                    ...
                    const pathResult = result?.paths.find(
                      (p) => p.from === from?.label && p.to === to?.label
                    );
                    return (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        {pathResult && (
                          <span
                            className={cn(
                              "font-mono",
                              pathResult.significant
                                ? "text-[var(--success)]"
                                : "text-[var(--error)]"
                            )}
                          >
                            β={pathResult.coefficient.toFixed(3)}{" "}
                            {pathResult.significant ? "✓" : "✗"}
                          </span>
                        )}
                        <button
            ...
            <div className="flex gap-2">
              <Button
                onClick={handleRun}
                disabled={loading || !fileData || nodes.length < 2 || edges.length < 1}
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Play className="w-4 h-4 mr-2" /> 运行 SEM
                  </>
                )}
              </Button>
              <Button
          ...
          <div className="lg:col-span-2 space-y-4">
            <div
              className="glass-card p-4 min-h-[400px] relative overflow-hidden"
                  ...
                  return (
                    <g key={i}>
                      <path
                        ...
                        fill="none"
                        stroke={color}
                        strokeWidth={pathResult ? 2.5 : 1.5}
                        markerEnd="url(#arrow)"
                        strokeDasharray={pathResult ? "none" : "6 4"}
                      />
                      {pathResult && (
                        <text
                          x={midX}
                          y={midY - 8}
                          textAnchor="middle"
                          className="text-[11px] font-mono font-bold fill-current"
                          style={{ color }}
                        >
                    ...
                    >
                      ...
                      <div className="flex-1 h-1.5 rounded-full bg-[var(--bg-tertiary)] overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, Math.abs(p.coefficient) * 100)}%`,
                            background: p.significant ? "var(--success)" : "var(--error)",
                          }}
                        />
                      </div>
                      ...
                      <span
                        ...
                        style={{ color: p.significant ? "var(--success)" : "var(--error)" }}
                      >
