...
function calcRFM(
  ...
  const customers = new Map<
    string,
    { firstPurchase: Date; lastPurchase: Date; frequency: number; monetary: number }
  >();
  let asOf = new Date(0);
  ...
  for (const row of data) {
    ...
    const amount =
      typeof row[amountCol] === "number" ? (row[amountCol] as number) : Number(row[amountCol]) || 1;
    if (date > asOf) asOf = date;
    if (!customers.has(cid))
      customers.set(cid, { firstPurchase: date, lastPurchase: date, frequency: 0, monetary: 0 });
  ...
  const n = rfm.length;
  const rScore = (val: number) => {
    const idx = recencySorted.findIndex((r) => r.recency === val);
    return idx < n * 0.2 ? 5 : idx < n * 0.4 ? 4 : idx < n * 0.6 ? 3 : idx < n * 0.8 ? 2 : 1;
  };
  const fScore = (val: number) => {
    const idx = frequencySorted.findIndex((r) => r.frequency === val);
    return idx < n * 0.2 ? 5 : idx < n * 0.4 ? 4 : idx < n * 0.6 ? 3 : idx < n * 0.8 ? 2 : 1;
  };
  const mScore = (val: number) => {
    const idx = monetarySorted.findIndex((r) => r.monetary === val);
    return idx < n * 0.2 ? 5 : idx < n * 0.4 ? 4 : idx < n * 0.6 ? 3 : idx < n * 0.8 ? 2 : 1;
  };
  ...
  const scored = rfm.map((r) => {
    const activeDays = Math.max(
      1,
      Math.ceil((r.last_purchase.getTime() - r.first_purchase.getTime()) / 86400000)
    );
...
export default function CLVPage() {
  ...
  const handleFileUpload = (data: ParsedData) => {
    ...
    setDateCol(
      headers.find((h, i) => /date|time|日期|时间/i.test(lc[i])) ?? headers[1] ?? headers[0]
    );
    setAmountCol(
      headers.find((h, i) => /amount|revenue|price|value|金额|消费/i.test(lc[i])) ??
        headers[2] ??
        headers[0]
    );
  };
  ...
  const handleRun = () => {
    ...
    try {
      ...
      addRecord({
        ...
        result: JSON.stringify(res.segments),
      });
    } catch {
      setResult(null);
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "你是一个客户价值分析专家。根据 RFM 分析结果，输出：1) 客户分群解读 2) 针对每个分群的营销策略建议 3) CLV 提升方案 4) 流失预警建议。用中文输出，结构化为 JSON。",
          },
          { role: "user", content: JSON.stringify(result) },
        ],
        stream: true,
        onChunk: (t) => setAiResult((prev) => prev + t),
      });
      if (!aiResult) setAiResult(res);
    } catch (e: unknown) {
      setAiResult(`错误: ${e instanceof Error ? e.message : "未知错误"}`);
    }
    setLoading(false);
  };

  const segmentData = result
    ? Object.entries(result.segments).map(([name, count]) => ({ name, count }))
    : [];
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-lg bg-[#14B8A6]/10 flex items-center justify-center text-xl">
            💰
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">客户价值分析 (CLV)</h1>
            <p className="text-sm text-[var(--text-muted)]">
              基于 Lifetimes · PyMC-Marketing · RFM 方法论
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-4">
          上传交易数据 → 自动计算 RFM/CLV → 客户分群 → 流失预警 → AI 生成营销策略
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <FileUpload
              ...
              description="CSV，需包含客户ID、交易日期、金额列"
            />
              ...
              <div className="glass-card p-4 space-y-3">
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Play className="w-4 h-4 mr-2" /> 运行 RFM / CLV 分析
                    </>
                  )}
                </Button>
              </div>
            ...
            {result && (
              <>
                <div className="glass-card p-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                    <div className="text-center">
                      ...
                      <div className="text-[10px] text-[var(--text-muted)]">平均 R (天)</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-[var(--text-primary)]">
                        {result.avg_frequency}
                      </div>
                      <div className="text-[10px] text-[var(--text-muted)]">平均 F (次)</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-[var(--text-primary)]">
                        {result.avg_monetary}
                      </div>
                      <div className="text-[10px] text-[var(--text-muted)]">平均 M (元)</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-[var(--text-primary)]">
                        {result.avg_predicted_clv}
                      </div>
                      <div className="text-[10px] text-[var(--text-muted)]">平均 12M CLV</div>
                    </div>
                  </div>
                </div>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-[var(--border)]">
                          ...
                          <th className="text-right py-1.5 px-2 text-[var(--text-muted)]">
                            12M CLV
                          </th>
                          <th className="text-right py-1.5 px-2 text-[var(--text-muted)]">
                            流失风险
                      ...
                      <tbody>
                        {result.top_10.map((c) => (
                          <tr key={c.customer_id} className="border-b border-[var(--border)]">
                            ...
                            <td className="text-right py-1.5 px-2 font-bold text-[var(--primary)]">
                              {c.rfm_score}
                            </td>
                            <td className="text-right py-1.5 px-2 text-[var(--text-primary)]">
                              {c.clv_12m}
                            </td>
                            <td className="text-right py-1.5 px-2 text-[var(--text-secondary)]">
                              {c.churn_risk}
                            </td>
                          </tr>
                        ))}
                ...
                <div className="glass-card p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-2 text-[10px] text-[var(--text-muted)]">
                      {Object.entries(result.churn_risk).map(([risk, count]) => (
                        <span key={risk}>
                          {risk}: {count}
                        </span>
                      ))}
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full h-10 border-[var(--border)] text-[var(--text-secondary)]"
                >
