...
function runPLSSEM(
  ...
  for (const lv of latentVars) {
    constructScores[lv.name] = Array.from(
      { length: n },
      (_, i) => lv.items.reduce((s, item) => s + (data[i][item] ?? 0), 0) / lv.items.length
    );
  }
  ...
  for (const lv of latentVars) {
    for (const item of lv.items) {
      ...
      let num = 0,
        ...
        d2 = 0;
      for (let i = 0; i < n; i++) {
        const a = itemVals[i] - mi,
          b = constructVals[i] - mc;
        num += a * b;
        d1 += a * a;
        d2 += b * b;
      }
  ...
  const fit: SEMResult["fit"] = {
    ...
    cfi_interp: cfi > 0.95 ? "良好" : cfi > 0.9 ? "可接受" : "不佳",
...
export default function SEMPage() {
  ...
  const addLatentVar = () => {
    ...
    const items = newVarItems
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (items.length < 2) {
      setError("潜变量需要至少 2 个测量指标");
      return;
    }
  ...
  const handleRun = () => {
    if (!fileData || latentVars.length < 1) {
      setError("请上传数据并定义至少 1 个潜变量");
      return;
    }
    if (paths.length < 1) {
      setError("请定义至少 1 条结构路径");
      return;
    }
    setLoading(true);
    setError("");
    ...
    try {
      ...
      addRecord({
        tool: "sem",
        type: "PLS-SEM",
        input: `${latentVars.length} LV, ${paths.length} paths`,
        result: JSON.stringify(res.fit),
      });
    } catch (e: unknown) {
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-lg bg-[#8B5CF6]/10 flex items-center justify-center text-xl">
            🔗
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">结构方程模型 (SEM)</h1>
            <p className="text-sm text-[var(--text-muted)]">
              基于 SmartPLS / AMOS 方法论 · PLS-SEM 算法
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-6">
          上传数据 → 定义潜变量 → 指定路径 → 运行 PLS-SEM → 查看路径系数、拟合指标、信效度
        </p>
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card p-4">
              ...
              <FileUpload
                ...
                description="CSV 文件，每行一个样本，列名为量表题项"
              />
              {fileData && (
            ...
            <div className="glass-card p-4">
              ...
              <div className="space-y-2 mb-3">
                <Input
                  placeholder="潜变量名称，如：感知价值"
                  value={newVarName}
                  onChange={(e) => setNewVarName(e.target.value)}
                  className="h-8 bg-[var(--bg-card)] border-[var(--border)] text-[var(--text-primary)]"
                />
                <Input
                  placeholder="测量指标（逗号分隔），如：PV1, PV2, PV3"
                  value={newVarItems}
                  onChange={(e) => setNewVarItems(e.target.value)}
                  className="h-8 bg-[var(--bg-card)] border-[var(--border)] text-[var(--text-primary)]"
                />
                <Button
                  size="sm"
                  onClick={addLatentVar}
                  className="h-8 bg-[var(--primary)] text-white"
                >
              ...
              <div className="space-y-2">
                {latentVars.map((lv) => (
                  <div
                    key={lv.name}
                    className="flex items-center gap-2 p-2 rounded-lg bg-[var(--bg-card)]"
                  >
                    ...
                    <button
                      onClick={() => removeLatentVar(lv.name)}
            ...
            <div className="glass-card p-4">
              ...
              <div className="flex gap-2 mb-3">
                ...
                >
                  ...
                  {latentVars.map((lv) => (
                    <option key={lv.name} value={lv.name}>
                      {lv.name}
                    </option>
                  ))}
                </select>
              ...
              <div className="space-y-1">
                {paths.map((p) => (
                  <div
                    key={`${p.from}-${p.to}`}
                    className="flex items-center gap-2 p-2 rounded-lg bg-[var(--bg-card)] text-sm"
                  >
                    <span className="text-[var(--primary)]">{p.from}</span>
                    <span className="text-[var(--text-muted)]">→</span>
                    <span className="text-[var(--accent)]">{p.to}</span>
                    <button
                      onClick={() => removePath(p.from, p.to)}
                      className="ml-auto text-[var(--text-muted)] hover:text-[var(--error)]"
                    >
                      <X className="w-3 h-3" />
                    </button>
            ...
            <Button
              onClick={handleRun}
              disabled={loading || !fileData || latentVars.length < 1 || paths.length < 1}
            ...
            >
              {loading ? (
                ...
                <>
                  <Play className="w-4 h-4 mr-2" /> 运行 PLS-SEM
                </>
              )}
            </Button>
              ...
              <>
                ...
                <div className="glass-card p-4">
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-3">
                    {[
                      {
                        label: "RMSEA",
                        value: result.fit.rmsea,
                        interp: result.fit.rmsea_interp,
                        good: result.fit.rmsea < 0.08,
                      },
                      {
                        label: "CFI",
                        value: result.fit.cfi,
                        interp: result.fit.cfi_interp,
                        good: result.fit.cfi > 0.9,
                      },
                      {
                        label: "SRMR",
                        value: result.fit.srmr,
                        interp: result.fit.srmr_interp,
                        good: result.fit.srmr < 0.08,
                      },
                    ].map((m) => (
                      <div key={m.label} className="text-center p-2 rounded-lg bg-[var(--bg-card)]">
                        <div
                          className="text-lg font-bold"
                          style={{ color: m.good ? "var(--success)" : "var(--warning)" }}
                        >
                          {m.value.toFixed(3)}
                        </div>
                ...
                <div className="glass-card p-4">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {result.measurement.map((m, i) => (
                          <tr key={i} className="border-b border-[var(--border)]">
                            <td className="py-2 px-2 text-[var(--primary)] font-medium">
                              {m.variable}
                            </td>
                            <td className="py-2 px-2 text-[var(--text-secondary)] font-mono">
                              {m.item}
                            </td>
                            <td
                ...
                <div className="glass-card p-4">
                  ...
                  <div className="space-y-2">
                    {result.structural.map((s, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-card)]"
                      >
                        ...
                        <div className="flex-1 flex items-center gap-2">
                          ...
                          <span
                            ...
                            style={{
                              color: s.significant ? "var(--success)" : "var(--text-muted)",
                            }}
                          >
                        ...
                        >
                          {s.p < 0.001 ? "p < .001" : `p = ${s.p.toFixed(3)}`}{" "}
                          {s.significant ? "✓" : "✗"}
                        </Badge>
                      </div>
                    ))}
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-[var(--border)]">
                          ...
                          <th className="text-center py-2 px-2 text-[var(--text-muted)]">
                            CR ≥ .7
                          </th>
                          <th className="text-center py-2 px-2 text-[var(--text-muted)]">
                            AVE ≥ .5
                      ...
                      <tbody>
                        {result.reliability.map((r, i) => (
                          <tr key={i} className="border-b border-[var(--border)]">
                            ...
                            <td
                              className="text-right py-2 px-2 font-mono"
                              style={{ color: r.cr_ok ? "var(--success)" : "var(--error)" }}
                            >
                              {r.cr.toFixed(3)}
                            </td>
                            <td
                              className="text-right py-2 px-2 font-mono"
                              style={{ color: r.ave_ok ? "var(--success)" : "var(--error)" }}
                            >
                              {r.ave.toFixed(3)}
                            </td>
                ...
                <ResultsExporter
                  ...
                  apa={{
                    ...
                    interpretation: `PLS-SEM 分析（N = ${fileData?.rowCount ?? 0}）显示模型拟合${result.fit.rmsea < 0.08 && result.fit.cfi > 0.9 ? "可接受" : "需要改进"}。${result.structural.filter((s) => s.significant).length}/${result.structural.length} 条路径显著。`,
                  }}
