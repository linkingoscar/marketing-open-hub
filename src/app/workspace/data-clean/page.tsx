...
const ACTIONS: { id: CleaningAction; label: string; icon: React.ElementType; desc: string }[] = [
  {
    ...
    label: "计算构念得分",
    icon: Calculator,
    desc: "多题项合并为均值（如 trust_1~5 → trust_score）",
  },
  {
    id: "reverse",
    label: "反向题 Recode",
    icon: RefreshCw,
    desc: "反转量表值（1→7, 2→6, ... 7→1）",
  },
...
export default function DataCleanPage() {
  ...
  const handleComposite = () => {
    ...
    const newData = {
      ...fileData,
      rows: newRows,
      headers: [...fileData.headers, compositeName],
      colCount: fileData.colCount + 1,
    };
  ...
  const handleReverse = () => {
    ...
    const newData = {
      ...fileData,
      rows: newRows,
      headers: [...fileData.headers, `${reverseCol}_rev`],
      colCount: fileData.colCount + 1,
    };
    setResultData(newData);
  ...
  const handleMissing = () => {
    ...
    if (missingStrategy === "drop") {
      ...
      newRows = newRows.filter((row) =>
        fileData.headers.every((h) => row[h] !== null && row[h] !== undefined && row[h] !== "")
      );
      ...
      for (const col of numCols) {
        ...
        if (filled > 0)
          addLog(
            `  "${col}": 用${missingStrategy === "mean" ? "均值" : "中位数"}填补了 ${filled} 个缺失值`
          );
      }
    }
  ...
  const handleDownload = () => {
    ...
    const csv = [
      data.headers.join(","),
      ...data.rows.map((r) =>
        data.headers.map((h) => `"${String(r[h] ?? "").replace(/"/g, '""')}"`).join(",")
      ),
    ].join("\n");
    ...
    a.href = url;
    a.download = `cleaned-${data.fileName}`;
    a.click();
              ...
              <div className="glass-card p-4 space-y-3">
                {action === "composite" && (
                  <>
                    ...
                    <Button
                      ...
                      disabled={!compositeName || compositeCols.length < 2}
                      className="w-full h-9 bg-[var(--primary)] text-white"
                    >
                      <Calculator className="w-4 h-4 mr-1" /> 计算构念得分（{compositeCols.length}{" "}
                      题项）
                    </Button>
                  </>
                )}
                ...
                {action === "reverse" && (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <select
                        value={reverseCol}
                        onChange={(e) => setReverseCol(e.target.value)}
                      ...
                      <Input
                        type="number"
                        value={scaleMax}
                        onChange={(e) => setScaleMax(Number(e.target.value) || 7)}
                        placeholder="量表最大值"
                        className="bg-[var(--bg-card)] border-[var(--border)] text-[var(--text-primary)]"
                      />
                    </div>
                ...
                {action === "rename" && (
                  <>
                        ...
                        return (
                          <div key={h} className="grid grid-cols-[auto_1fr] gap-2 items-center">
                            <button
                              ...
                              onClick={() =>
                                setDroppedCols((prev) =>
                                  dropped ? prev.filter((c) => c !== h) : [...prev, h]
                                )
                              }
                              className={cn(
                            ...
                            <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2 items-center">
                              ...
                              <Input
                                ...
                                disabled={dropped}
                                onChange={(e) =>
                                  setRenameMap((prev) => ({ ...prev, [h]: e.target.value }))
                                }
                    ...
                    <Button
                      onClick={handleColumnManage}
                      disabled={droppedCols.length >= fileData.headers.length}
                      className="w-full h-9 bg-[var(--primary)] text-white"
                    >
            ...
            {log.length > 0 && (
              <div className="glass-card p-4">
                ...
                <div className="space-y-1 max-h-[150px] overflow-y-auto">
                  {log.map((l, i) => (
                    <p key={i} className="text-xs text-[var(--text-secondary)] font-mono">
                      {l}
                    </p>
                  ))}
                </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-[var(--text-tertiary)]">
                {resultData
                  ? `处理后（${resultData.rowCount} 行 × ${resultData.colCount} 列）`
                  : fileData
                    ? "原始数据预览"
                    : "等待上传数据"}
              </span>
                  ...
                  <thead>
                    <tr className="border-b border-[var(--border)]">
                      {(resultData || fileData)!.headers.map((h) => (
                        <th
                          key={h}
                          className="text-left py-2 px-2 text-[var(--text-muted)] font-medium sticky top-0 bg-[var(--bg-secondary)]"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {(resultData || fileData)!.rows.slice(0, 50).map((row, i) => (
                      <tr key={i} className="border-b border-[var(--border)]">
                        {(resultData || fileData)!.headers.map((h) => (
                          <td
                            key={h}
                            className="py-1.5 px-2 text-[var(--text-secondary)] max-w-[150px] truncate"
                          >
                            {String(row[h] ?? "")}
                          </td>
                        ))}
