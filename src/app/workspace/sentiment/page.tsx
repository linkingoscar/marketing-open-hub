...
const ANALYSIS_TYPES = [
  {
    ...
    icon: "💬",
    prompt:
      "对以下文本进行情感分析，输出 JSON：{sentiment, score, emotions, aspects, summary, insights}",
  },
  {
    id: "theme",
    label: "主题提取",
    icon: "🏷️",
    prompt:
      "从以下文本中提取主要主题和话题，输出 JSON：{themes: [{name, frequency, keywords, sentiment}]}",
  },
  {
    id: "persona",
    label: "用户画像",
    icon: "👤",
    prompt:
      "根据以下消费者数据，生成用户画像，输出 JSON：{personas: [{name, age_range, occupation, pain_points, motivations, channels}]}",
  },
  {
    id: "competitor",
    label: "竞品分析",
    icon: "⚔️",
    prompt:
      "根据以下信息进行竞品分析，输出 JSON：{competitors: [{name, strengths, weaknesses, positioning}], gaps, recommendations}",
  },
  {
    id: "purchase-intent",
    label: "购买意向",
    icon: "🛒",
    prompt:
      "模拟 5 种消费者人格对产品的购买意向，输出 JSON：{personas: [{type, intent_score, concerns, likelihood, quote}]}",
  },
...
export default function SentimentToolPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (hasFile) {
        ...
        const texts = fileData!.rows
          .map((r, i) => `${i + 1}. ${String(r[col] ?? "")}`)
          .filter((t) => t.length > 3);
  ...
  const downloadResult = () => {
    ...
    a.href = url;
    a.download = `analysis-${analysisType}-${Date.now()}.md`;
    a.click();
  ...
  const downloadCSV = () => {
    ...
    const csv = [
      ...
      ...fileData.rows.map((r) =>
        fileData.headers.map((h) => `"${String(r[h] ?? "").replace(/"/g, '""')}"`).join(",")
      ),
    ].join("\n");
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-muted)]">
              基于 Sentique · CustomerInsight · Sentiment-Research-Report · Google Social Pulse
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-4">
          上传数据文件或粘贴文本，选择分析类型，AI 输出结构化结果
        </p>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <FileUpload
                ...
                description="支持 CSV / JSON / TSV，自动识别文本列"
              />
            </div>
                      ...
                      <tr className="border-b border-[var(--border)]">
                        {fileData.headers.map((h) => (
                          <th
                            key={h}
                            className={cn(
                              "text-left py-1.5 px-2 text-[var(--text-muted)] font-medium",
                              h === textColumn && "text-[var(--primary)]"
                            )}
                          >
                            {h} {h === textColumn && "✦"}
                          </th>
                      ...
                      {fileData.rows.slice(0, 5).map((row, i) => (
                        <tr key={i} className="border-b border-[var(--border)]">
                          {fileData.headers.map((h) => (
                            <td
                              key={h}
                              className="py-1.5 px-2 text-[var(--text-secondary)] max-w-[200px] truncate"
                            >
                              {String(row[h] ?? "")}
                            </td>
                          ))}
            ...
            <Button
              onClick={handleRun}
              disabled={loading || (!textInput.trim() && !fileData) || !hasAnyKey()}
