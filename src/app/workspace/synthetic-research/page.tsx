...
const PRESETS = [
  {
    ...
    label: "购买意向测试",
    systemPrompt:
      "你是一个消费者调研专家。请根据以下产品描述，从 5 个不同消费者视角（价格敏感型、品质追求型、早期采用者、保守型、随机型）分别给出：1) 购买意向评分(1-5) 2) 一句话理由 3) 最关心的 3 个问题。输出 JSON 格式。",
    placeholder: "描述你的产品/服务，例如：一款 AI 写作助手，月费 $29，帮助营销人员生成社媒内容...",
  },
  {
    id: "concept-test",
    label: "概念测试",
    systemPrompt:
      "你是一个市场研究专家。请对以下产品概念进行评估：1) 目标人群画像 2) 核心价值主张 3) 潜在竞品 4) 定价建议 5) 上市风险。每个维度给出 3-5 个要点。",
    placeholder: "描述你的产品概念...",
  },
  {
    id: "sentiment-analysis",
    label: "情感分析",
    systemPrompt:
      "你是一个 NLP 情感分析专家。请对以下文本进行分析：1) 整体情感(正面/中性/负面) 2) 情感分数(-1到1) 3) 关键情感词 4) 潜在意图 5) 一句话摘要。输出 JSON 格式。",
    placeholder: "输入要分析的文本（评论、社媒帖子等）...",
  },
  {
    id: "competitor-analysis",
    label: "竞品分析",
    systemPrompt:
      "你是一个竞争情报分析师。请根据以下信息进行竞品分析：1) 直接竞品列表 2) 各竞品优劣势 3) 市场空白点 4) 差异化建议 5) 定位策略。",
  ...
  },
  {
    ...
    label: "用户画像生成",
    systemPrompt:
      "你是一个用户研究专家。请根据以下产品信息生成 3 个典型用户画像：每个画像包含 姓名/年龄/职业/收入/痛点/使用场景/决策因素/信息获取渠道。输出 JSON 格式。",
    placeholder: "描述你的产品和目标用户...",
  },
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-muted)]">
              基于 synthetic-market-research / MicroCrowd / Tunnel 方法论
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-4">
          输入产品概念，AI 模拟多个人格消费者给出反馈、评分和建议
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <Button
              onClick={handleRun}
              disabled={loading || !userInput.trim() || !hasAnyKey()}
          ...
          <div className="space-y-4">
            ...
            <div
              className={cn(
                "glass-card p-6 min-h-[300px] max-h-[600px] overflow-y-auto",
