...
function runUplift(
  ...
  const ate =
    treated.reduce((s, r) => s + r[outcomeCol], 0) / treated.length -
    control.reduce((s, r) => s + r[outcomeCol], 0) / control.length;

  const featureImpacts: Record<string, number> = {};
  for (const f of featureCols) {
    const highT = treated.filter(
      (r) => r[f] > treated.reduce((s, x) => s + x[f], 0) / treated.length
    );
    const highC = control.filter(
      (r) => r[f] > control.reduce((s, x) => s + x[f], 0) / control.length
    );
    if (highT.length > 0 && highC.length > 0) {
      featureImpacts[f] =
        highT.reduce((s, r) => s + r[outcomeCol], 0) / highT.length -
        highC.reduce((s, r) => s + r[outcomeCol], 0) / highC.length;
    }
  }
...
function classifyPost(value: string | number, allValues: (string | number)[]): boolean {
  if (typeof value === "string" && /(post|after|后|之后|干预后|1|true|yes)$/i.test(value.trim()))
    return true;
...
function simpleRegressionSSE(x: number[][], y: number[]) {
  ...
  const beta = matVecMul(
    matInv(XtX.map((row, i) => row.map((v, j) => v + (i === j ? 1e-8 : 0)))),
    matVecMul(transpose(X), y)
  );
...
function runGranger(
  ...
  const rows = data
    .map((r, i) => ({
      t: timeCol ? r[timeCol] : i,
      x: toNum(r[causeCol]),
      y: toNum(r[outcomeCol]),
    }))
    .filter((r): r is { t: string | number; x: number; y: number } => r.x !== null && r.y !== null)
    .sort((a, b) => {
      const da = Date.parse(String(a.t)),
        db = Date.parse(String(b.t));
  ...
  return {
    ...
    interpretation:
      p < 0.05
        ? `${causeCol} 的滞后值显著提升了 ${outcomeCol} 的预测力，可认为存在 Granger 因果关系。`
        : `${causeCol} 的滞后值未显著提升 ${outcomeCol} 的预测力，未发现 Granger 因果关系。`,
  };
}
...
export default function CausalPage() {
  ...
  const handleFileUpload = (data: ParsedData) => {
    ...
    if (numCols.length >= 2) {
      setTreatmentCol(numCols[0]);
      setOutcomeCol(numCols[1]);
    }
    setTimeCol(
      data.headers.find((h) => /time|date|period|week|month|日期|时间|期次|阶段/i.test(h)) ??
        data.headers[2] ??
        ""
    );
  };
  ...
  const handleRun = async () => {
    ...
    if (method === "causal-ai") {
          ...
          messages: [
            {
              ...
              content:
                customPrompt ||
                "你是一个因果推断专家。分析数据中的因果关系，输出：1) 可能的因果路径 2) 混杂变量识别 3) 因果效应估计 4) 建议的因果分析方法。用中文输出。",
            },
      ...
      try {
        const featureCols = fileData.headers.filter(
          (h) =>
            h !== treatmentCol &&
            h !== outcomeCol &&
            h !== timeCol &&
        ...
        const numericRows = fileData.rows.map((r) => {
          ...
          for (const h of [treatmentCol, outcomeCol, ...featureCols]) {
            const v = r[h];
            row[h] = typeof v === "number" ? v : Number(v) || 0;
          }
          return row;
        });
        const res =
          method === "uplift"
            ...
            : method === "did"
              ? runDiD(fileData.rows, treatmentCol, outcomeCol, timeCol)
              : method === "scm"
                ? runSyntheticControl(fileData.rows, treatmentCol, outcomeCol, timeCol)
                : method === "granger"
                  ? runGranger(fileData.rows, treatmentCol, outcomeCol, timeCol)
                  : runUplift(numericRows, treatmentCol, outcomeCol, featureCols);
        setResult(res);
        addRecord({
          tool: "causal",
          type: currentMethod?.label ?? method,
          input: `${treatmentCol} → ${outcomeCol}`,
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-muted)]">
              基于 CausalML · DoWhy · EconML · TFCausalImpact
            </p>
          </div>
        </div>
        <p className="text-[var(--text-secondary)] mb-4">
          上传数据 → 选择因果分析方法 → 估计处理效应 → 识别混杂变量
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <FileUpload
              ...
              description="CSV，需包含处理变量（0/1）和结果变量"
            />
              ...
              <div className="glass-card p-4 space-y-3">
                ...
                <Button
                  ...
                  disabled={
                    loading ||
                    !fileData ||
                    (method !== "causal-ai" && (!treatmentCol || !outcomeCol))
                  }
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Play className="w-4 h-4 mr-2" /> 运行分析
                    </>
                  )}
                </Button>
              </div>
              ...
              {result ? (
                <div className="space-y-3">
                  <h3 className="text-sm font-medium text-[var(--text-primary)] mb-3">
                    {String(result.method ?? selectedMethod?.label ?? "因果分析")}结果
                  </h3>
                  {Object.entries(result)
                    .filter(([k]) => k !== "feature_impacts" && k !== "interpretation")
                    .map(([k, v]) => (
                      <div
                        key={k}
                  ...
                  {result.feature_impacts && (
                    <div className="mt-3">
                      ...
                      {Object.entries(result.feature_impacts as Record<string, number>).map(
                        ([f, v]) => (
                          <div key={f} className="flex items-center gap-2 mb-1">
                            ...
                            <div className="flex-1 h-1.5 bg-[var(--bg-tertiary)] rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, Math.abs(v as number) * 50)}%`,
                                  background: (v as number) > 0 ? "var(--success)" : "var(--error)",
                                }}
                              />
                            </div>
