...
const PROVIDER_META: Record<
  string,
  { icon: string; color: string; desc: string; recommended: string; features: string[] }
> = {
  openai: {
    icon: "🟢",
    color: "#10B981",
    desc: "GPT-4o 系列，全球最广泛使用的 LLM",
    recommended: "gpt-4o",
    features: ["函数调用", "视觉理解", "JSON 输出"],
  },
  anthropic: {
    icon: "🟠",
    color: "#F97316",
    desc: "Claude 系列，擅长长文本和推理",
    recommended: "claude-sonnet-4-20250514",
    features: ["200K 上下文", "工具调用", "安全对齐"],
  },
  gemini: {
    icon: "🔵",
    color: "#3B82F6",
    desc: "Google Gemini，多模态原生支持",
    recommended: "gemini-2.0-flash",
    features: ["多模态", "长上下文", "免费额度"],
  },
  mimo: {
    icon: "🟣",
    color: "#8B5CF6",
    desc: "小米 MiMo，国产旗舰推理模型",
    recommended: "mimo-v2.5-pro",
    features: ["1M 上下文", "深度思考", "多模态"],
  },
  deepseek: {
    icon: "🟢",
    color: "#10B981",
    ...
    features: ["1M 上下文", "思考模式", "超低价格"],
  },
  qwen: {
    icon: "🟠",
    color: "#F59E0B",
    desc: "阿里通义千问，国产全能选手",
    recommended: "qwen3.6-plus",
    features: ["1M 上下文", "内置工具", "批量推理"],
  },
  kimi: {
    icon: "🟡",
    color: "#FBBF24",
    desc: "月之暗面 Kimi，长程代码和多模态",
    recommended: "kimi-k2.6",
    features: ["256K 上下文", "视觉理解", "深度思考"],
  },
  doubao: {
    icon: "🔴",
    color: "#EF4444",
    desc: "字节跳动豆包，Seed 2.0 系列",
  ...
  spark: {
    icon: "🟣",
    color: "#A855F7",
    desc: "讯飞星火，X1.5 快思考模式",
    recommended: "4.0Ultra",
    features: ["32K 上下文", "联网搜索", "免费 Lite"],
  },
  zhipu: {
    icon: "🟢",
    color: "#14B8A6",
    desc: "智谱 GLM，GLM-5.1 旗舰",
    recommended: "glm-5.1",
    features: ["200K 上下文", "深度思考", "代码优化"],
  },
  custom: {
    icon: "⚙️",
    color: "#64748B",
    desc: "任意 OpenAI 兼容端点",
    recommended: "",
    features: ["自定义 Base URL", "自定义模型", "完全灵活"],
  },
};
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-3xl font-bold mb-2">
          <span className="gradient-text">API 设置</span>
        </h1>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          >
            {configuredCount > 0 ? (
              <>
                <Check className="w-3 h-3 mr-1" /> {configuredCount} 个已配置
              </>
            ) : (
              "未配置"
            )}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {hasKey && (
                        <Badge
                          variant="outline"
                          className="text-[9px] border-[var(--success)]/30 text-[var(--success)]"
                        >
                          已配置
                        </Badge>
                      )}
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {provider.models.map((m) => (
                              <button
                                key={m}
                                onClick={() => setConfig(provider.id, { model: m })}
                                className={cn(
                              ...
                              >
                                ...
                                {m === meta.recommended && (
                                  <span className="block text-[9px] text-[var(--warm)] mt-0.5">
                                    ⭐ 推荐
                                  </span>
                                )}
                              </button>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center gap-2 text-xs text-[var(--success)]">
                              ...
                              {config.usage.lastLatencyMs && (
                                <span className="text-[var(--text-muted)]">
                                  · 延迟 {config.usage.lastLatencyMs}ms
                                </span>
                              )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                              {[
                                {
                                  label: "请求数",
                                  value: config.usage.totalRequests.toLocaleString(),
                                  icon: "📊",
                                },
                                {
                                  label: "总 Tokens",
                                  value:
                                    config.usage.totalTokens >= 1000000
                                      ? `${(config.usage.totalTokens / 1000000).toFixed(1)}M`
                                      : config.usage.totalTokens >= 1000
                                        ? `${(config.usage.totalTokens / 1000).toFixed(1)}K`
                                        : String(config.usage.totalTokens),
                                  icon: "🔤",
                                },
                                {
                                  label: "预估费用",
                                  value: `${provider.pricing?.unit?.includes("¥") ? "¥" : "$"}${config.usage.estimatedCost.toFixed(2)}`,
                                  icon: "💰",
                                },
                                {
                                  label: "上次使用",
                                  value: config.usage.lastUsed
                                    ? new Date(config.usage.lastUsed).toLocaleDateString("zh-CN", {
                                        month: "short",
                                        day: "numeric",
                                        hour: "2-digit",
                                        minute: "2-digit",
                                ...
                                <div
                                  key={m.label}
                                  className="p-2 rounded-lg bg-[var(--bg-card)] text-center"
                                >
                                  <div className="text-[10px] text-[var(--text-muted)]">
                                    {m.icon} {m.label}
                                  </div>
                                  <div className="text-sm font-mono font-medium text-[var(--text-primary)] mt-0.5">
                                    {m.value}
                                  </div>
                                </div>
                              ))}
                            ...
                            {config.usage.totalTokens > 0 && (
                              <div>
                                <div className="flex items-center justify-between text-[10px] text-[var(--text-muted)] mb-1">
                                  <span>Token 分布</span>
                                  <span>
                                    {config.usage.inputTokens.toLocaleString()} 输入 /{" "}
                                    {config.usage.outputTokens.toLocaleString()} 输出
                                  </span>
                                </div>
                                <div className="h-2 rounded-full bg-[var(--bg-tertiary)] overflow-hidden flex">
                                  <div
                                    ...
                                    style={{
                                      width: `${(config.usage.inputTokens / config.usage.totalTokens) * 100}%`,
                                    }}
                                  />
                                  <div
                                    className="h-full bg-[var(--accent)]"
                                    style={{
                                      width: `${(config.usage.outputTokens / config.usage.totalTokens) * 100}%`,
                                    }}
                                  />
                                </div>
                                <div className="flex justify-between text-[9px] text-[var(--text-muted)] mt-0.5">
                                  <span>
                                    <span className="inline-block w-2 h-2 rounded-full bg-[var(--primary)] mr-1" />
                                    输入
                                  </span>
                                  <span>
                                    <span className="inline-block w-2 h-2 rounded-full bg-[var(--accent)] mr-1" />
                                    输出
                                  </span>
                                </div>
                              </div>
                            ...
                            {provider.pricing && (
                              <div className="text-[10px] text-[var(--text-muted)]">
                                💡 定价：输入 {provider.pricing.input} / 输出{" "}
                                {provider.pricing.output} {provider.pricing.unit}
                              </div>
                            )}
                          </div>
                        )}
                          ...
                          <div className="mt-3">
                            <button
                              onClick={async () => {
                                ...
                                setTestResults((prev) => ({
                                  ...prev,
                                  [provider.id]: undefined as unknown as ConnectionTestResult,
                                }));
                              ...
                              >
                                {testResults[provider.id].success ? (
                                  <span className="text-[var(--success)]">
                                    ✓ 连接成功 · 延迟 {testResults[provider.id].latencyMs}ms · 模型:{" "}
