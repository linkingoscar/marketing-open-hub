...
function StackedCard({ project, index, parentRef }: {
  ...
  const { scrollYProgress } = useScroll({
    target: parentRef,
    offset: [`${index * 0.15} center`, `${(index + 1) * 0.15} center`],
  });

  const initialY = 60 * index;
  const initialScale = 1 - index * 0.04;
  const initialOpacity = 0.6;

  const y = useTransform(scrollYProgress, [0, 1], [index === 0 ? 0 : initialY, 0]);
  const scale = useTransform(scrollYProgress, [0, 1], [index === 0 ? 1 : initialScale, 1]);
