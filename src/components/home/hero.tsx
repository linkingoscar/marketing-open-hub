  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <span ref={ref} className="text-2xl sm:text-3xl font-bold text-[var(--text-primary)]">
          {count}
          {suffix}
        </span>
      </div>
...
export function Hero() {
  const typewriterText = useTypewriter(
    ["消费者行为分析", "情感分析", "用户画像", "营销归因", "品牌监测", "需求验证"],
    80,
    40,
    1800
  );
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        >
          <span>🚀</span>
          <span>发现 31 个精选营销研究开源项目，覆盖 9 大核心方向</span>
        </motion.div>
        ...
        >
          ...
          <span className="ml-2 text-[var(--primary-light)] font-medium font-mono">
            {typewriterText}
          </span>
        ...
        <div className="flex items-center justify-center gap-8 sm:gap-12">
          {stats.map((stat, i) => (
            <StatCounter key={stat.label} {...stat} delay={1.0 + i * 0.1} />
          ))}
        </div>
      </div>
