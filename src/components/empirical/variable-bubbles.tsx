...
export function VariableBubbles({
  ...
  onRemoveItem,
  onMergeItems,
  onRemoveConstruct: _onRemoveConstruct,
  selectedConstructs,
  onSelectConstruct,
  ...
  return (
    <div ref={containerRef} className="relative space-y-6" onClick={() => setContextMenu(null)}>
        ...
        {mergeMode && mergeSelection.length >= 2 && (
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="新构念名称"
              className="h-7 px-2 rounded text-xs bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text-primary)]"
            />
            <button
              onClick={handleMerge}
              className="h-7 px-3 rounded text-xs bg-[var(--primary)] text-white"
            >
              合并 ({mergeSelection.length})
            </button>
          </div>
        )}
      </div>
            ...
            >
                  ...
                  >
                    ...
                    {!mergeMode && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          onRemoveItem(construct.id, item.name);
                        }}
              ...
              <div className="mt-3 relative z-10">
                <div className="flex items-center justify-between text-[10px] text-[var(--text-muted)] mb-1">
                  ...
                  <span>
                    {(
                      construct.meanScore.reduce((s, v) => s + v, 0) / construct.meanScore.length
                    ).toFixed(2)}
                  </span>
                </div>
      ...
      {demographics.length > 0 && (
        <div>
          <span className="text-xs text-[var(--text-muted)] mb-2 block">
            人口统计学变量 ({demographics.length})
          </span>
          <div className="flex flex-wrap gap-2">
            {demographics.map((item) => (
              <span
                key={item.name}
                className="px-2.5 py-1 rounded-full text-xs border border-[var(--border)] text-[var(--text-muted)]"
              >
        ...
        {contextMenu && (
          ...
          >
            {contextMenu.constructId && (
              <button
                className="w-full px-3 py-2 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-card-hover)] text-left flex items-center gap-2"
                onClick={() => {
                  onRemoveItem(contextMenu.constructId!, contextMenu.itemName);
                  setContextMenu(null);
                }}
              >
            ...
            <button
              ...
              onClick={() => {
                setMergeMode(true);
                setMergeSelection([contextMenu.itemName]);
                setContextMenu(null);
              }}
            >
