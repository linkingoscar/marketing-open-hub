...
function getSlotPositions(slots: FrameworkSlot[]): Record<string, { x: number; y: number }> {
  ...
  // IV on the left
  iv.forEach((s, i) => {
    positions[s.id] = { x: 80, y: centerY + (i - (iv.length - 1) / 2) * 100 };
  });
  // DV on the right
  dv.forEach((s, i) => {
    positions[s.id] = { x: 720, y: centerY + (i - (dv.length - 1) / 2) * 100 };
  });
  // Mediators in the center
  mediators.forEach((s, i) => {
    positions[s.id] = { x: 400, y: centerY + (i - (mediators.length - 1) / 2) * 100 };
  });
  // Moderators above/below
  moderators.forEach((s, i) => {
    positions[s.id] = { x: 400, y: centerY - 140 + i * 280 };
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card p-6 overflow-x-auto">
        ...
        >
          ...
          {template.slots.map((slot) => {
            ...
            const assignedConstruct = assignedId
              ? constructs.find((c) => c.id === assignedId)
              : null;
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  r="38"
                  fill={
                    assignedConstruct ? `${assignedConstruct.color}15` : "rgba(255,255,255,0.03)"
                  }
                  stroke={
                    isHovered
                      ? roleColor
                      : assignedConstruct
                        ? assignedConstruct.color
                ...
                <circle
                  ...
                  r="28"
                  fill={
                    assignedConstruct ? `${assignedConstruct.color}20` : "rgba(255,255,255,0.02)"
                  }
                  stroke="none"
                />
                {/* Content */}
                {assignedConstruct ? (
                  <>
                    <text
                      ...
                      fill="#F1F5F9"
                    >
                      {assignedConstruct.displayName.length > 8
                        ? assignedConstruct.displayName.slice(0, 7) + "…"
                        : assignedConstruct.displayName}
                    </text>
                    <text
                      x={pos.x}
                      y={pos.y + 12}
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={() => onRunAnalysis(assignments)}
