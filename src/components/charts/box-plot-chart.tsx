        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {chartData.map((d, i) => {
              const g = groups[i];
              const sd = Math.sqrt(
                g.data.reduce((s, v) => s + (v - d.mean) ** 2, 0) / (g.data.length - 1)
              );
              return (
                <tr key={i} className="border-b border-[var(--border)]">
                  <td className="py-1 px-2 text-[var(--text-primary)]">{d.label}</td>
                  <td className="text-right py-1 px-2 font-mono text-[var(--text-secondary)]">
                    {d.n}
                  </td>
                  <td className="text-right py-1 px-2 font-mono text-[var(--text-primary)]">
                    {d.mean.toFixed(2)}
                  </td>
                  <td className="text-right py-1 px-2 font-mono text-[var(--text-secondary)]">
                    {sd.toFixed(2)}
