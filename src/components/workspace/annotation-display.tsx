  ...
  return (
    <div className={cn("glass-card p-4 space-y-3", className)}>
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
          {annotation.practicalImplication}
        </p>
      </div>
          ...
          <ul className="space-y-1">
            {annotation.actionItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-xs text-[var(--text-secondary)]">
                <span className="w-4 h-4 rounded-full bg-emerald-400/10 flex items-center justify-center text-[9px] font-bold text-emerald-400 shrink-0 mt-0.5">
                  {i + 1}
                </span>
                {item}
              </li>
