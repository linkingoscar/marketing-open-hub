    ...
    <div className="glass-card p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-7 px-2 text-xs text-[var(--text-muted)] hover:text-[var(--error)]"
          onClick={() => {
            if (confirm("清空所有历史记录？")) clearAll();
          }}
        >
        ...
        <div className="space-y-2 max-h-[300px] overflow-y-auto">
          {toolRecords.map((record) => (
            <div
              key={record.id}
              className={cn(
                "p-3 rounded-lg border border-[var(--border)] cursor-pointer hover:bg-[var(--bg-card-hover)] transition-colors",
              ...
              {selectedId === record.id && (
                <div className="mt-2 pt-2 border-t border-[var(--border)]">
                  <p className="text-xs text-[var(--text-secondary)] line-clamp-3 mb-2">
                    输入: {record.input.slice(0, 200)}
                    {record.input.length > 200 ? "..." : ""}
                  </p>
