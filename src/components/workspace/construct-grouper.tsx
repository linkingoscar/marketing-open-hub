...
export function ConstructGrouper({
  columns,
  groups,
  onChange,
  minGroups = 2,
  maxGroups = 6,
}: ConstructGrouperProps) {
  ...
  return (
    <div className="space-y-3">
                  ...
                  style={
                    selected
                      ? {
                          ...
                          borderColor: groupColors[idx % groupColors.length],
                        }
                      : undefined
                  }
                >
      ...
      {/* Unassigned hint */}
      {unassigned.length > 0 && (
        <p className="text-[10px] text-[var(--text-muted)]">未分组：{unassigned.join(", ")}</p>
      )}
    </div>
  );
}
