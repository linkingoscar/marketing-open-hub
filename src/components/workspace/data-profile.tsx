...
function analyzeColumn(name: string, rows: Record<string, string | number>[]): ColumnProfile {
  ...
  if (type === "numeric" && numericVals.length >= 2) {
    ...
    const sd = Math.sqrt(
      numericVals.reduce((s, v) => s + (v - m) ** 2, 0) / (numericVals.length - 1)
    );
    const sorted = [...numericVals].sort((a, b) => a - b);
    const n = sorted.length;
    const skew =
      (n / ((n - 1) * (n - 2))) * numericVals.reduce((s, v) => s + ((v - m) / sd) ** 3, 0);
    profile.stats = {
      mean: +m.toFixed(2),
      sd: +sd.toFixed(2),
      min: sorted[0],
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "缺失率",
            value: `${totalCells > 0 ? ((totalMissing / totalCells) * 100).toFixed(1) : 0}%`,
            color: totalMissing > 0 ? "#F59E0B" : "#10B981",
          },
          ...
          <tbody>
            {profiles.map((p) => (
              ...
              >
                ...
                <td className="text-center py-2 px-2 font-mono text-[var(--text-secondary)]">
                  {p.n}
                </td>
                <td className="text-center py-2 px-2">
                  <span
                ...
                <td className="py-2 px-2 text-[var(--text-muted)] max-w-[200px] truncate">
                  {p.sampleValues.join(", ")}
                </td>
              </tr>
            ))}
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isMissing ? "bg-[var(--error)]/60" : "bg-[var(--success)]/20"
                        )}
