...
import { Star, GitFork, Globe, TrendingUp, ArrowLeft } from "lucide-react";
...
// ===== Score Radar Chart =====
function ScoreRadar({ scores }: { scores: Project["scores"] }) {
  const data = [
...
export function ProjectDetailClient({ project, catName, catColor, avgScore, related }: ProjectDetailClientProps) {
  return (
    <>
      {/* Back */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <a href={`/category/${project.category}`} className="inline-flex items-center gap-1 text-sm text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors mb-6">
          <ArrowLeft className="w-4 h-4" /> 返回{catName}
        </a>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <Badge variant="outline" className="border-[var(--border)] text-[var(--text-muted)]">{catName}</Badge>
        </div>
